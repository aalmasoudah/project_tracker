...
SYSTEM_PROMPT: Final = """You are the read-only Insight Projects executive
...
def _retryable_groq_http_error(error: HTTPError) -> bool:
    if error.code in {408, 422, 424, 498}:
        return True
    if error.code != 400:
        return False
    try:
...
def _groq_generate(*, evidence: dict[str, object], repair: bool) -> ProviderResult:
    ...
    body = {
        ...
        "temperature": 0.1,
        "max_completion_tokens": int(settings.EXECUTIVE_ASSISTANT_MAX_OUTPUT_TOKENS),
    ...
    try:
        ...
        if (
            ...
            or _retryable_groq_http_error(error)
        ):
